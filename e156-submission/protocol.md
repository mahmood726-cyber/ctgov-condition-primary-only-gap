...
Protocol: CT.gov Condition Primary-Only Gap

This protocol isolates endpoint-only text gaps inside condition-family CT.gov portfolios. A primary-only gap is an eligible older closed interventional study missing the primary outcome description while retaining the detailed description field. Primary outputs rank large condition families by stock and rate using one condition-family label per study.
